...
# ╔═╡ c1a64fd1-0e2f-4579-8d1e-a154e96e9f17
using Base: Text
...
md"""
`rs_pair` takes a permutation as a vector or a string \
`rsk_pair` takes a generalized permutation or a matrix
"""
...
begin
	...
	#s = collect("3182574") #see https://en.wikipedia.org/wiki/Robinson%E2%80%93Schensted_correspondence#/media/File:Young-Schensted.png
	Text(String(s)) # Display the string without quotes
end

# ╔═╡ 4fe386fa-6f4b-4670-949f-b0de133907f6
md"""
Larger numbers (characters) to the right \
Otherwise bump and insert (ties to the right) 
"""

# ╔═╡ b4fcb4ce-5474-49e5-81c6-0cf6a2024643
@htl """
Show Record (Q Matrix) $(@bind show_Q CheckBox())
"""

# ╔═╡ f2070389-11f7-422a-b074-fa3678e2c5de
md"""
# Standard and Semi-Standard
"""

# ╔═╡ aee8d53a-15ce-40a3-9ed6-087c0f4e419e
# Permutation's are standard
rsk_pair(I(4)[randperm(4),:])

# ╔═╡ f0308f24-517b-4bbe-b9ed-526cc9e27d2a
# Generalized Permutations aare semi-standard
let
	 Z = rand(0:2,4,4)
	 display(Z)
	 display(YoungTableaux.twoline_array(Z))
     rsk_pair(Z)
end

# ╔═╡ 7100e331-cd21-4796-b1f5-f13d212ac970
# If you have one 1 in each row you are semi-standard, standard
let 
	v = rand(1:5,5)
	Z = zeros(Int,5,5)
	for i=1:5, Z[i,v[i]] = 1 end
	display(Z)
	display(YoungTableaux.twoline_array(Z))
    display(typeof(rsk_pair(Z)))
end
...
md"""
Note some of the basic code is in [YoungTableaux.jl](https://github.com/simeonschaub/YoungTableaux.jl)
"""
...
# ╔═╡ 5ef3bf50-20f4-4df5-8a19-7bae79c97b85
md"""
# Logan-Shepp Limit
"""
...
# ╔═╡ 88519eb5-3618-4fc5-973c-46e82a548a00
md"""
# Richard Stanley Example (just for fun)
"""

# ╔═╡ f9536da7-8743-43d2-a9b8-b432ead8c99d
md"""
Recreation of [https://math.mit.edu/events/stanley70/savethedate/](https://math.mit.edu/events/stanley70/savethedate/):
"""
...
begin
	...
	Y = getindex.(eachindex(Z), 1)
	# Logan-Shepp Limit
	# -2 <= u <- 2
	u = (X .- Y) ./ √N  # scale by cardinality of partition
	v = (X .+ Y) ./ √N

	fig = Figure()
	ax = Axis(fig[1, 1]; limits = ((-2.5, 2.5), (0, 2.5)), aspect = DataAspect())
  	scatter!(ax, collect(u), collect(v);
		marker = :circle, space = :data, markersize = 150/√N, color = :transparent,
		strokewidth = 0.5, strokecolor = :black,
	)
	lines!(ax, -2..2, ℓ -> 2/pi * (ℓ * asin(ℓ/2) + sqrt(4 - ℓ^2));
