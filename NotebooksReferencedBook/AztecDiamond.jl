...
# ╔═╡ 3a1d1e1e-89bc-11ed-1194-eb3974e2539a
using OffsetArrays, Transducers, Folds, Images, Colors, PlutoUI

# ╔═╡ 9abe27b0-3d4a-4217-8154-70139084f478
using Transducers: @next, complete, Executor

# ╔═╡ b9009aab-c422-4cfe-af93-6aa8e081f841
begin
	using CairoMakie
	#CairoMakie.activate!(type="svg")
	using Makie.GeometryBasics: Vec2f, Point2f, Rect2f
	using CairoMakie: Axis
	using PlutoUI: Button, Slider, CheckBox
	using HypertextLiteral, AbstractPlutoDingetjes.Bonds
end

# ╔═╡ fa509627-626b-4c55-940d-13b8cd3c0ca9
using LinearAlgebra, SpecialFunctions, FastGaussQuadrature, ForwardDiff

# ╔═╡ 5dc6a288-8d7a-42b9-8165-3a89d2fc217c
using Statistics

# ╔═╡ f51bb935-e687-4143-a7d5-e3651bc12867
TableOfContents(indent=true, depth=4, aside=true)

# ╔═╡ b6250277-b415-4001-a724-8184661bcbf3
@enum Edge::UInt8 NONE UP RIGHT

# ╔═╡ 942683e0-1199-4e55-a21f-e1246a6a368a
inds(N) = (1-N:N, 1-N:N)

# ╔═╡ 3c86dbce-23fe-45f5-ba76-1e0d22ccc5a7
begin
...
# ╔═╡ 6a6f6d25-1bba-4ad4-aa56-9bc0796e8233
md"""
	This notebook was originally created by Simeon Schaub and modified by Alan Edelman
"""

# ╔═╡ 5996c3cb-4398-4a54-876b-8e286e7dd40b
begin
...
# ╔═╡ 6e73675b-6b70-44fd-b4d6-04f1b0775dc2
begin
...
# ╔═╡ 2eab4fcd-e52a-4753-8a65-4c2de0a441f6
begin
...
# ╔═╡ 5ef6bea9-56eb-41c7-b594-8241b717d360
md"""
## Aztec Diamond
"""


# ╔═╡ 7243e740-7641-4749-8888-71f5e2a1c3ea
...
# ╔═╡ a12aef01-b2eb-4f52-9e62-114cb4352cbf
function prepare_plot(t::Tiling; pad=.1f0)
	...
	foreach(faces(t)) do (i, j, isdotted)
		if t[i, j] == UP
			r = Rect2f(i-1+pad, j-1+pad, 2-2pad, 1-2pad)
			col = isdotted ? colorant"red" : colorant"yellow"
			push!(tiles, r)
			push!(colors, col)
			off = isdotted ? -.3f0 : .3f0
			push!(arrow_pts, Point2f(i, j-.5f0-off))
			push!(arrows, Point2f(0, isdotted ? -.5f0 : .5f0))
		elseif t[i, j] == RIGHT
			r = Rect2f(i-1+pad, j-1+pad, 1-2pad, 2-2pad)
			col = isdotted ? colorant"green" : colorant"blue"
			push!(tiles, r)
			push!(colors, col)
			off = isdotted ? -.3f0 : .3f0
			push!(arrow_pts, Point2f(i-.5f0-off, j))
...
# ╔═╡ 6b4ee3a4-6faf-4ddc-9bd4-61cfa52cc5f8
md"""
## Algorithm 
"""

# ╔═╡ 8e4faad5-0d86-43d5-a3da-f3b058f26594
begin
...
# ╔═╡ fa26ce23-2f95-4496-9745-f8934dce3e1e
begin
	# destruction
	...
	# filling
	function fill_empty_blocks!(t′::Tiling; ex=SequentialEx())
	...
	function step!(t′::Tiling, t::Tiling; ex=SequentialEx())
		...
		slide_tiles!(t′, t; ex)
		fill_empty_blocks!(t′; ex)
		return t′
	end
end

# ╔═╡ ff639830-d988-4978-ae4a-1bd79fdc9e96
function diamond!(t, t′, N; ex=SequentialEx())
	for N in 1:N
		(; x) = t′
		fill!(view(x, inds(N-1)...), NONE)
		t′ = Tiling(N, x)
		t, t′ = step!(t′, t; ex), t
	end
	return t
...
# ╔═╡ 59665c82-017e-4c0a-9d37-7105d1022050
function diamond(N)
	t, t′ = Tiling(0; sizehint=N), Tiling(0; sizehint=N)
	return diamond!(t, t′, N)
end

# ╔═╡ 69ff9fa2-caf6-490c-88d4-0477d924417f
diamond(500) |> to_img

# ╔═╡ 26e3cda7-f9a3-4ec9-b2f1-5666c538579c
function steps_diamond(N)
	Channel() do ch
		t, t′ = Tiling(0; sizehint=N), Tiling(0; sizehint=N)
		for N in 1:N
			x = t′.x
			...
			put!(ch, remove_bad_blocks!(t))
			put!(ch, slide_tiles!(t′, t))
			put!(ch, fill_empty_blocks!(t′))
...
# ╔═╡ b03665fc-0e1f-432d-9325-3696546df491
STEPS = map(copy, steps_diamond(20));

# ╔═╡ c1a8d26d-3e07-4201-9595-a633672b21bf
md"""
Step through shuffling algorithm: $(@bind NTH_STEP SeekingSlider(4:60))

Show Arrows $(@bind SHOW_ARROWS CheckBox(true))
"""

# ╔═╡ e1933deb-5806-4a0a-80c8-1c3a7e584325
let
	f = Figure()
	t = STEPS[NTH_STEP]
	Axis(f[1, 1]; aspect=1)
	xlims!(-t.N, t.N)
	ylims!(-t.N, t.N)
	tiles, colors, arrow_pts, arrows = prepare_plot(t)
	poly!(tiles; color=colors)
	SHOW_ARROWS && arrows!(arrow_pts, arrows)
	f
end

# ╔═╡ bd26e724-fa33-45ba-b520-bf58a294996a
...
# ╔═╡ b4bf7fe8-df3f-485b-b580-221e0bd4b3bd
D = diamond(30);

# ╔═╡ 39e1bca1-3cd9-4aca-b10d-d8b49200c702
let (; N) = D
	f = Figure()
	Axis(f[1, 1]; aspect=1)
	xlims!(-N, N)
	ylims!(-N, N)
	tiles, colors, arrow_pts, arrows = prepare_plot(D)
	poly!(map(((; origin, widths),) -> Rect2f(reverse(origin), reverse(widths)), tiles); color=colors)
	#image!(permutedims(to_img(D)); interpolate=false)
	lines!(-N:N, parent(dr_path(D)); color=:black, linewidth=3)
	f
end

# ╔═╡ f48dd844-0b1f-4590-9454-6330c0409075
md"""
## Airy Process
"""

# ╔═╡ de243696-6cb4-4de0-9388-918bcbf1921e
@bind N_DR Slider(1:200; show_value=true)

# ╔═╡ 83d4e7dd-ffcb-437c-890d-b9ad356bd524
let n=N_DR
	D = diamond(n)
	Xₙ = dr_path(D)
	t = (2^(1/6) / n^(2/3)) .* (-n:n)
	𝒜 = map(parent(Xₙ), t) do Xₙ, t
		(Xₙ - n/√2) / (2^(-5/6) * n^(1/3)) + t^2
	end
	
	f = Figure()
	Axis(f[1, 1]; title=L"Airy Process from $X_n(t)$ for $n = %$n$")
	ylims!(-5, 5)
	lines!(t, 𝒜)
	f
end

# ╔═╡ 4f7165d9-4f80-4bc8-ae28-83c3115c832e
# ╠═╡ disabled = true
#=╠═╡
tw_data = map(1:200) do _		
	n=rand(150:200)
	D = diamond(n)
	Xₙ = dr_path(D)
	𝒜₀ = (Xₙ[0] - n/√2) / (2^(-5/6) * n^(1/3))
end
  ╠═╡ =#

# ╔═╡ c0a9682b-18ca-44e4-a7c6-ece08c754020
#=╠═╡
let
	f = Figure()
	Axis(f[1, 1]; title=L"$\mathcal{A}_n(0)$ for $n$ sampled between 150 and 200")
	xlims!(-5, 2)
	hist!(tw_data; normalization=:pdf, bins=range(-5, 2; length=31))
	x = range(-5, 2; length=101)
	lines!(x, pdf_tracy_widom.(x); color=:red, linewidth=2, label="PDF Tracy-Widom")
	axislegend()
	f
end
  ╠═╡ =#

# ╔═╡ 448e29aa-a7b0-4564-ad08-7b2c5da8ab94
let
...
# ╔═╡ 77ea178c-44e4-436b-a830-f0786559835f
# ╠═╡ disabled = true
#=╠═╡
airy_data, airy_t = let n=200, iters=200
	t = (2^(1/6) / n^(2/3)) .* OffsetVector(-n:n, -n:n)
	airy_data = OffsetMatrix{Float64}(undef, -n:n, iters)
	for i in 1:iters
		D = diamond(n)
		Xₙ = dr_path(D)
		map!(view(airy_data, :, i), Xₙ, t) do Xₙ, t
			(Xₙ - n/√2) / (2^(-5/6) * n^(1/3)) + t^2
		end
	end
	airy_data, t
...
# ╔═╡ 3309b180-943c-483a-abcb-a5f26e0c5d5b
#=╠═╡
let
	f = Figure()
	Axis(
		f[1, 1];
		title=L"$\mathrm{cov}(\mathcal{A}_n(0), \mathcal{A}_n(t))$ estimated using 200 samples with $n=200$",
		xlabel=L"t",
		ylabel=L"\mathrm{cov}(\mathcal{A}_n(0), \mathcal{A}_n(t))",
	)
	xlims!(-4, 4)
	ylims!(-0.05, 0.85)
	lines!(parent(airy_t), cov(parent(airy_data); dims=2)[:, 201])
	f
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "c13e95c141947063f68b7960542e251ce94845ec"
...
git-tree-sha1 = "20976fc9e26e64e3c2e12837a0d598f2fb4328b9"
