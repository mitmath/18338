...
# ╔═╡ 92a6d932-425c-11ed-2fd9-1dccd27a0b16
using LinearAlgebra, Random, Combinatorics, Plots, Distributions, PlutoUI

# ╔═╡ e5a5afcf-a7a2-4c99-b19d-10f0a09b3475
begin
	##################################################
...
# ╔═╡ eb29d970-3240-4bc4-9821-57895db771fe
begin
...
# ╔═╡ ef232523-109b-475f-9597-580194e0d86e
@bind n Slider(5:15; show_value=true)

# ╔═╡ 9bec8e89-a202-47f5-ae9f-8d976f597a14
begin
	K = kernel_aztec(n)
	size(K)
end

# ╔═╡ 924a07a3-6c81-421e-9701-d8e4a0b1d28f
s_poulson = sampleDPP(K; tol=1e-8);

# ╔═╡ 552f5224-aba3-4077-9779-6e5bf8e0ab75
s_peres = shuffle(s_poulson);

# ╔═╡ f627b369-a875-4542-977b-480514500ce3
K

# ╔═╡ fffb97cf-1e6a-4f2f-81b9-bed65ec44ecd
@bind t Slider(1:length(s_poulson); default=length(s_poulson))

# ╔═╡ a1b32cb1-2c7c-42b7-a742-08a18f8e7ced
t

# ╔═╡ f72b1600-6a82-439b-abba-172dc48c5ad7
dc1, rl1 = partial_diamond(s_poulson[1:t], n);

# ╔═╡ b1cd6928-db92-49d1-ac17-a25aea98ae75
dc2, rl2 = partial_diamond(s_peres[1:t], n);

# ╔═╡ e811c8f1-7a2e-4112-a941-0bfe401cfee8
begin
	heatmap(dc1, leg=false, title="Aztec Diamond n=$n at time t=$t", 
						color=c_codes[1:maximum(dc1)+1], 
	                    aspect_ratio=1, xlims=(1/2,2n+1/2), ylims=(1/2,2n+1/2), 
	                    showaxis=false, tick=false, size=(400, 400))
	plot!(rl1, legend=false, color=:transparent, linewidth=2, aspectratio=1)
end

# ╔═╡ 7ec24e96-7dad-461d-aa65-0d5f075f44a2
begin
	heatmap(dc2, leg=false, title="Aztec Diamond n=$n at time t=$t", 
						color=c_codes[1:maximum(dc2)+1], 
	                    aspect_ratio=1, xlims=(1/2,2n+1/2), ylims=(1/2,2n+1/2), 
	                    showaxis=false, tick=false, size=(400, 400))
	plot!(rl2, legend=false, color=:transparent, linewidth=2, aspectratio=1)
end
...
manifest_format = "2.0"
project_hash = "3d730520bff7705d5042b3bf4313711c1f82be4d"
