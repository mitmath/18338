...
using Combinatorics, Plots, PlutoUI, Symbolics, LinearAlgebra
...
begin 	
 ...
 for i=1:length(τ)
	 plot!( [τ[i][1],τ[i][end]], [levs[i],levs[i]],color=:black, lw=4)
	 for j∈τ[i]
		 plot!([j,j],[levs[i],m], color=:black, lw=4)
...
ζ[kperm,kperm] == ζ' # lattice anti-isomorphism

# ╔═╡ f7adcb14-9f79-4a34-99ab-007b39e73b9c
@variables r

# ╔═╡ 4bced153-d6f4-4376-aa0b-6d9f8f660cb9
c = r.^(length.(NC)  )

# ╔═╡ 9512d097-1522-4e2e-82a6-b9042d17e365


# ╔═╡ 5d87d263-6b6e-4f81-b64d-69ca543422fd
φ = ζ * c;

# ╔═╡ e1a83964-d1c1-4ef6-8c59-b4837d0a2653
substitute.(φ./r,r=>2) 

# ╔═╡ 2648986d-2a7f-498d-8d4f-9422b02159d8
let  # Wishart matrix moment
   r = 1/2
   m = 4000
   n  = round(Int,r*m)
   A = randn(m,n)
   ...
   f(τ) = prod(tr.([W] .^ length.(kreweras(τ)[1])))/m^(length(kreweras(τ)[1]))
	
   f.(NC)
   
end

# ╔═╡ d0e15e23-f5d1-47cb-938c-ff2d90acd9b8
let
 φ = (c' *ζ)[:] 
substitute.(φ./r,r=>2) 
end

# ╔═╡ 4ba0c696-5b21-4bab-ad32-bfba9bf9dc0e
let  # Wishart matrix moment
   r = 1/2
   m = 4000
   n  = round(Int,r*m)
   ...
   f(τ) = prod(tr.([W] .^ length.(τ)))/n^(length(τ))
	
   f.(NC).*r
   
end

# ╔═╡ 07303c7b-6b68-436d-aabd-67a8ca1fe42a
let  # Wishart matrix moment
   r = 2
   m = 4000
   n  = round(Int,r*m)
   ...
   f(τ) = prod(tr.([W] .^ length.(τ)))/m^(length(τ))
...
manifest_format = "2.0"
project_hash = "9917dfed9fe2451a025b2e1dd33706d8e8168c92"
...
git-tree-sha1 = "fde08c554e2f59d4640282f4448a804e18518e6a"
