...
# ╔═╡ 3edbeafa-d051-11f0-0f05-d14f9e76b6d5
using LinearAlgebra, CairoMakie, JLD2, LaTeXStrings, Optim

# ╔═╡ 61a5a068-92d6-4d42-a06e-d3ea650d68e1
set_theme!(theme_latexfonts())

# ╔═╡ 833b2dcd-4c2f-4855-af01-e5b3d3c5e77d
html"""
<style>
  main {
    max-width: 1000px;
...
# ╔═╡ 9318a62d-1002-4bda-9d5a-de6674fa6d8c
begin
...
# ╔═╡ fa5a1369-7401-4702-87b2-5221cb1872f6
...
# ╔═╡ cc1266cc-8dc0-42f2-a533-25f0a9d48401
function plot_comparison(ns, ks, t, p, suffix="")
	fontsize = 20
	...
	for m in 1:length(ks)
		for j in 1:length(ns)
			...
			l = length(data[m, :])
			currax = Axis(fig[1, (m - 1) * length(ns) + j], aspect=DataAspect(), spinewidth=0, xtickcolor=:lightgray, ytickcolor=:lightgray, xticklabelcolor=:gray, yticklabelcolor=:gray, xgridwidth=2.0, ygridwidth=2.0, title=latexstring("n=$(ns[j]),\\ k=$(k),\\ t=$(round(t, digits=2))"), titlesize=fontsize, titlecolor=:gray, width=300, height=300)
			for i in 1:l
	...
	for m = 1:length(ks)
		for j = 1:length(ns)
			...
			currax = content(fig[1, (m - 1) * length(ns) + j])
	...
	ax1 = content(fig[1, 1])
	#fig[length(ks)+1, 1:length(ns)] = Legend(fig, ax1, framevisible=false, labelsize=fontsize, labelcolor=:gray)
	fig[2, 1:(length(ns) * length(ks))] = Legend(fig, ax1, framevisible=false, labelsize=fontsize, labelcolor=:gray)
	rowsize!(fig.layout, 2, Relative(0.1))
	resize_to_layout!(fig)
	fig
end

# ╔═╡ 47b8dda3-6af9-4625-add0-55224deaf5ef
...
# ╔═╡ 4c903194-cf94-4900-abf1-9a62411205e4
md"""
# The Brown measure
"""

# ╔═╡ 9e877536-5036-41e9-9b79-1e74c57d0eb4
c1 = Makie.to_colormap(:imola10)

# ╔═╡ 62a96107-32f6-4235-8c97-8827a244e0c7
md"""
## Brown measure for ``k=6`` and varying ``t``
"""

# ╔═╡ a0015bab-004c-41da-a968-5617192e3187
plot_brown_measure(6, 3, plot_brown_measure(6, 2, plot_brown_measure(6, 1, nothing, c1[4]), c1[6]), c1[8])

# ╔═╡ a8986725-6f3b-4ed0-a179-7bad04fab8b7
plot_brown_measure(6, 4.5, plot_brown_measure(6, 4, plot_brown_measure(6, 3.5, nothing, c1[4]), c1[6]), c1[8])

# ╔═╡ 8531fdee-e3db-46e9-9194-fc3987ed31b6
md"""
## Brown measure for ``k=\infty`` and varying ``t``
"""

# ╔═╡ 3ac5c196-3a3d-44a5-84fb-2869ee8b956e
plot_brown_measure(10000, 3, plot_brown_measure(10000, 2, plot_brown_measure(10000, 1, nothing, c1[4]), c1[6]), c1[8])

# ╔═╡ e209657e-cd8a-490d-8ea5-a4473180d551
plot_brown_measure(10000, 4.5, plot_brown_measure(10000, 4, plot_brown_measure(10000, 3.5, nothing, c1[4]), c1[6]), c1[8])

# ╔═╡ d9e7dc7d-4cf1-427f-bb1f-70d1e24ac891
md"""
## Brown measure and eigenvalues of a random walk with ``n=1000, k=6, t=2``
"""

# ╔═╡ 1b33b760-d2bd-49be-a918-3272c30d02eb
begin
	bmtestevs = load("data/bmtestevs.jld2")["data"]
	figbeannoexp = plot_brown_measure(6, 2, nothing, Makie.to_colormap(:imola10)[7])
	scatter!(real(bmtestevs), imag(bmtestevs), color=:black, markersize=5)
	#save("bean_no_exp.pdf", figbeannoexp)
	figbeannoexp
end

# ╔═╡ 6d1b4eb6-039b-4a0d-9eb8-26056d7fb77e
md"""
# Numerical Experiments on the extremal eigenvalue
"""

# ╔═╡ 2534b941-2a2b-4c9a-af8e-fba6fb0c686a
md"""
## Distribution of ``\varsigma_{700, 3}^{\mathfrak{gl}, (j)}(1)``
"""

# ╔═╡ 0350a71d-759e-4c79-a106-558a2ee26126
plot_evs_and_brown_measure(700, 3, 1.0, "g", "g_700_3_1.pdf")

# ╔═╡ 1966e640-32f9-4ebd-9000-96e67a286d00
md"""
## Distribution of ``\varsigma_{700, 3}^{\mathfrak{gl}, (j)}(2)``
"""

# ╔═╡ c3eecf99-575f-4d78-b562-d5d2381428ba
plot_evs_and_brown_measure(700, 3, 2.0, "g", "g_700_3_2.pdf")

# ╔═╡ e2a732b0-ed55-4ca6-987f-eac76b45a530
md"""
## Distribution of ``\sigma_{700, 3}^{\mathfrak{gl}, (j)}(1)``
"""

# ╔═╡ c59e62e1-1107-408f-b7be-94ec75aff57a
plot_evs_and_brown_measure(700, 3, 1.0, "g", "g_700_3_1_ne.pdf", "_ne")

# ╔═╡ b334327b-aee0-45b1-aa3d-7ca1e7f95a31
md"""
## Distribution of ``\sigma_{700, 3}^{\mathfrak{gl}, (j)}(2)``
"""

# ╔═╡ 1bbe9bc0-276e-4e14-8947-b68b5e9ec87d
plot_evs_and_brown_measure(700, 3, 2.0, "g", "g_700_3_2_ne.pdf", "_ne")

# ╔═╡ b817eabf-95f5-40ea-81c8-4972764b27c0
md"""
## Distribution of ``\sigma_{700, 6}^{\mathfrak{gl}, (j)}(1)``
"""

# ╔═╡ 31cb9662-6d11-4414-912e-e3877d6adddd
plot_evs_and_brown_measure(700, 6, 1.0, "g", "g_700_6_1_ne.pdf", "_ne")

# ╔═╡ 0f84f4cd-7d09-4a43-8e4b-8fe355b43988
md"""
## Distribution of ``\sigma_{700, 6}^{\mathfrak{gl}, (j)}(2)``
"""

# ╔═╡ f6fa5cbd-716a-4d99-8188-0509583defe2
plot_evs_and_brown_measure(700, 6, 2.0, "g", "g_700_6_2_ne.pdf", "_ne")

# ╔═╡ c9cff035-4a81-4450-9b9e-bb95ac542f78
md"""
# Varying the parameters for ``\varsigma_{n, k}^{\mathfrak{gl}, (j)}(t)`` and ``\sigma_{n, k}^{\mathfrak{gl}, (j)}(t)``
"""

# ╔═╡ fcc7ca67-6b39-4e70-b53b-6ca496cb7a23
md"""
## ``\varsigma_{n, 3}^{\mathfrak{gl}, (j)}(1)`` for different ``n``
"""

# ╔═╡ df62a01a-bb47-4648-8c64-06487e772cb4
plot_comparison([50, 100, 150], [3], 1, "g")

# ╔═╡ d6635dd9-9113-466b-a874-2e40051251f0
md"""
## ``\varsigma_{200, k}^{\mathfrak{gl}, (j)}(1)`` for different ``k``
"""

# ╔═╡ 0a47cb0b-1dfa-4660-8a01-42828b65bb53
plot_comparison([200], [3, 6, 20], 1, "g")

# ╔═╡ 66476a32-d6cd-4dff-8c83-914b1d545b63
md"""
## ``\sigma_{n, 3}^{\mathfrak{gl}, (j)}(1)`` for different ``n``
"""

# ╔═╡ c8383e0d-4f5e-4f36-a0b7-d896c51a599d
plot_comparison([50, 100, 150], [3], 1, "g", "_ne")

# ╔═╡ 5e0ea703-19d8-4103-84d1-d7008cd7a0b2
md"""
## ``\sigma_{150, k}^{\mathfrak{gl}, (j)}(1)`` for different ``k``
"""

# ╔═╡ ae8ee795-dac3-43f7-b95b-a8fc8b09e922
plot_comparison([200], [3, 6, 20], 1, "g", "_ne")
...
manifest_format = "2.0"
project_hash = "853baabc2f4dbe92e28378e6a6daad745499ab62"
